...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    ...
    const messages = dataaa.map((item, index) => {
      return `\n${index + 1}. 👤 𝘕𝘢𝘮𝘦: ${item.artist}\n📜 𝘛𝘪𝘵𝘭𝘦: ${item.title}\n⏳ 𝘛𝘪𝘮𝘦: ${item.timestamp} giây`;
    });
...
setTimeout(async () => {
        ...
        api.sendMessage({
          body: `[ 𝐒𝐎𝐔𝐍𝐃𝐂𝐋𝐎𝐔𝐃 - 𝐌𝐏𝟑 ]\n────────────────────\n[👤] → 𝘕𝘢𝘮𝘦: ${chosenItem.artist}\n[📝] → 𝘛𝘪𝘵𝘭𝘦: ${chosenItem.title}\n[⏳] → 𝘛𝘪𝘮𝘦: ${chosenItem.timestamp} giây\n[🔈] → 𝘏𝘪𝘵 𝘱𝘭𝘢𝘺: ${chosenItem.views}\n[🗓️] → 𝘜𝘱𝘭𝘰𝘢𝘥: ${chosenItem.release}\n[📶] → 𝘚𝘱𝘦𝘦𝘥 𝘉𝘪𝘵: ${bit}\n────────────────────\n[⏰] → Time: ${moment.tz("Asia/Ho_Chi_Minh").format("DD/MM/YYYY || HH:mm:ss")}`,
